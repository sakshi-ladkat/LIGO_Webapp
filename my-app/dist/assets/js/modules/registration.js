...
window.isRestoringForm = false;
window.lastUserSelectedContinent = null;
...
async function setContinentValue(continentId) {
  ...
  if (window.lastProcessedContinentId === continentId) return;

  window.isProgrammaticChange = true;
  window.lastProcessedContinentId = continentId;
  window.lastUserSelectedContinent = continentId;

  continentSelect.value = continentId;

  if (window.currentLoadedContinentId !== continentId) {
    await loadCountries();
  }
...
function saveFormData() {
  ...
  const existingStep = parseInt(
    sessionStorage.getItem(STORAGE.STEP) || "1",
    10,
  );
...
async function restoreFormData(skipStepRestore = false) {
  if (window.isRestoringForm) return;

  window.isRestoringForm = true;
  window.isProgrammaticChange = true;
  ...
  try {
    ...
    const shouldSkipStepRestore =
      skipStepRestore || (hasTokenInUrl && savedStepNum <= 3);
...
async function checkURLParams() {
  ...
  if (window.location.hash.includes("?")) {
    ...
    console.log("[checkURLParams] Parsed from hash:", {
      token: !!token,
      email,
      error,
    });
  ...
  if (!token && !email && !error && window.location.search) {
    ...
    email = urlParams.get("email");
    error = urlParams.get("error");
    console.log("[checkURLParams] Parsed from search:", {
      token: !!token,
      email,
      error,
    });
  ...
  if (currentStep >= 4 && !hasUrlParams) {
    console.log(
      "[checkURLParams] On Step 4+, no URL params to process. Skipping redirect logic.",
    );
    restoreFormData();
    return;
  }
  ...
  if (token && email) {
    if (sessionStorage.getItem("last_verified_token") === token) {
      console.log(
        "Token already verified in this session, skipping re-verification logic",
      );
      ...
      if (currentStep <= 3) {
        console.log(
          "[checkURLParams] User on Step 1-3, navigating to Step 3 to show verification",
        );
        goToStep(3);
        restoreFormData(true);
      } else {
        console.log(
          "[checkURLParams] User already on Step 4+, staying on current step",
        );
...
async function loadInstitutes() {
  ...
  const sessionCache = JSON.parse(
    sessionStorage.getItem("reference_data_cache") || "{}",
  );
...
function populateInstitutesSelect(select, institutes) {
  ...
  if (saved.institute_id) {
    select.value = saved.institute_id;
  }
  window.handleInstituteChange();
}

window.handleInstituteChange = function () {
  const instituteSelect = document.getElementById("institute");
  const wrapper = document.getElementById("otherInstituteWrapper");
  if (!instituteSelect || !wrapper) return;

  if (instituteSelect.value === "other") {
    wrapper.style.display = "block";
  } else {
    wrapper.style.display = "none";
    const otherInput = document.getElementById("otherInstitute");
    if (otherInput) otherInput.value = "";
    hideFieldError("otherInstitute");
  }
};
...
async function loadContinents(force = false) {
  ...
  window.continentsLoadPromise = (async () => {
    ...
    const now = Date.now();
    let cFetchStart = parseInt(
      sessionStorage.getItem("_continentFetchWindowStart") || "0",
      10,
    );
    let cFetchCount = parseInt(
      sessionStorage.getItem("_continentFetchCount") || "0",
      10,
    );
    ...
    if (cFetchCount > 10) {
      console.error(
        "[loadContinents] HARD LOOP DETECTED. Blocking API calls for 10 seconds.",
      );
      sessionStorage.setItem(
        "_continentFetchWindowStart",
        (now + 10000).toString(),
      );
      return;
    }
    ...
    try {
      const res = await fetch(
        `${CONFIG.API_BASE_URL}/api/reference/continents`,
        {
  ...
  if (!continentChangeHandlerAttached) {
    select.addEventListener("change", async function () {
      ...
      if (
        ...
        window.isLoadingCountries ||
        window.isRestoringForm
      )
        return;

      // 🚨 MOST IMPORTANT FIX
      if (window.lastUserSelectedContinent === selectedValue) {
        console.log("[Continent] Duplicate event blocked");
        return;
      }

      window.lastUserSelectedContinent = selectedValue;

      console.log("[Continent] User changed → loading countries");
...
async function loadCountries() {
  ...
  const now = Date.now();
  let fetchStart = parseInt(
    sessionStorage.getItem("_countryFetchWindowStart") || "0",
    10,
  );
  let fetchCount = parseInt(
    sessionStorage.getItem("_countryFetchCount") || "0",
    10,
  );
  ...
  if (fetchCount > 10) {
    console.error(
      "[loadCountries] HARD LOOP DETECTED. Blocking API calls for 10 seconds.",
    );
    sessionStorage.setItem(
      "_countryFetchWindowStart",
      (now + 10000).toString(),
    ); // Block for 10 secs
    return;
  }
  ...
  if (currentCountriesLoadPromise && currentFetchingContinentId === continentId)
  ...
  currentCountriesLoadPromise = (async () => {
    ...
    try {
      ...
      if (e.name !== "AbortError") {
        ...
        if (countrySelect)
          countrySelect.innerHTML = "<option>-- Error --</option>";
      }
    } finally {
      window.isLoadingCountries = false;
...
function initializeAutoSave() {
  Object.values(FIELD_MAP).forEach((fid) => {
    ...
    if (fid === "country") {
      const handler = () => {
        if (!window.isProgrammaticChange) saveFormData();
      };
    ...
    if (fid === "email") {
      ...
      el.addEventListener("blur", function () {
        if (
          ...
          this.value !== lastFetchedDraftEmail
        )
          getDraft(this.value);
      });
    }
...
async function saveDraft() {
  ...
  if (window.registrationCurrentStep)
    formData.current_step = window.registrationCurrentStep;
...
async function getDraft(email) {
  if (!email || email === lastFetchedDraftEmail || window._isFetchingDraft)
    return;
  window._isFetchingDraft = true;

  try {
    const res = await fetch(
      `${CONFIG.API_BASE_URL}/api/registration/draft/${email}`,
    );
    if (!res.ok) return;
    ...
    if (draft) {
      ...
      const currentData = JSON.parse(
        sessionStorage.getItem(STORAGE.FORM) || "{}",
      );
      sessionStorage.setItem(
        STORAGE.FORM,
        JSON.stringify({ ...currentData, ...draft }),
      );
    }
  } catch (e) {
...
async function sendVerificationEmail() {
  ...
  const sendBtn = step3
    ? step3.querySelector('button[onclick*="sendVerificationEmail"]')
    : null;
  let originalText = "";
  ...
  try {
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/api/registration/send-verification`,
      {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      ...
      // Polling to handle cross-tab verification: Automatically advances the UI
      // if the user clicks the link in a different tab.
      if (window.verificationPollInterval)
        clearInterval(window.verificationPollInterval);

      window.verificationPollInterval = setInterval(async () => {
        if (window.registrationCurrentStep !== 3) {
          clearInterval(window.verificationPollInterval);
          return;
        }
        try {
          ...
          if (
            data.draft &&
            (data.draft.is_verified || data.draft.verified_email)
          ) {
            clearInterval(window.verificationPollInterval);

            verificationToken = data.draft.token;
            verifiedEmail = email;

            sessionStorage.setItem(STORAGE.TOKEN, verificationToken);
            sessionStorage.setItem(STORAGE.EMAIL, verifiedEmail);
            sessionStorage.setItem("last_verified_token", verificationToken);
            sessionStorage.setItem(STORAGE.VERIFIED_STATUS, "true");

            const emailVerifiedDiv = document.getElementById("emailVerified");
            const emailNotVerifiedDiv =
              document.getElementById("emailNotVerified");
            const nextBtn = document.getElementById("emailNextBtn");
            const verifiedInput = document.getElementById("verifiedEmail");
            ...
            if (window.showToast)
              window.showToast(
                "Email verified in another tab! Continuing...",
                "success",
              );
            else if (window.toastr)
      ...
      if (
        ...
        if (window.toastr) {
          ...
          window.showToast(
            data.message + " (Check console to resend)",
            "warning",
          );
...
async function resendVerification(email) {
  ...
  try {
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/api/registration/resend-verification`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      toastr.success(
        "Verification link sent again. Please check your email to continue.",
      );
    } else {
...
function goToStep(step) {
  ...
  if (instituteSelect) {
    ...
    if (step > 1 && step < 5) {
      let selectedText =
        instituteSelect.options[instituteSelect.selectedIndex]?.text;

      if (instituteSelect.value === "other") {
        const otherInput = document.getElementById("otherInstitute");
        if (otherInput && otherInput.value.trim() !== "") {
          selectedText = otherInput.value.trim();
        }
      }
      ...
      if (selectedText && selectedText !== "-- Select Institute --") {
        if (!banner) {
          ...
          banner.style.cssText =
            "background: #f8fafc; padding: 12px 16px; border: 1px solid #e2e8f0; font-weight: 500; color: #475569; margin-bottom: 20px; border-radius: 8px; display: flex; align-items: center; gap: 10px; user-select: none; box-shadow: 0 1px 2px rgba(0,0,0,0.05);";
...
function validateStep(step) {
  switch (step) {
    case 1:
      const instValid = validateField(
        "institute",
        "Please select an institute",
      );
      const instValue = document.getElementById("institute")?.value;
      if (instValid && instValue === "other") {
    ...
    case 3:
      if (!verificationToken || !verifiedEmail) {
        ...
        if (window.showToast)
          ...
          window.toastr.error("Please verify your email before proceeding");
...
function multiStepRegisterMount() {
  ...
  const now = Date.now();
  if (
    window.registrationMountedTimestamp &&
    now - window.registrationMountedTimestamp < 2000
  ) {
  ...
  if (savedStep) {
    ...
    console.log(
      "[Mount] Restored step from storage:",
      window.registrationCurrentStep,
    );
  } else {
    window.registrationCurrentStep = 1;
...
window.handleInstituteChange = handleInstituteChange;

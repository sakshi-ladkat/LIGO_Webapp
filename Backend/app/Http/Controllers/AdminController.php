...
class AdminController extends Controller
{
    ...
    private function old_applicationTracker(Request $request, int $id): JsonResponse
    {
        if ($err = $this->checkAdmin($request))
            return $err;
            ...
            if ($approvals->has($step->workflow_step_id)) {
                ...
                if ($approval->recommended_services) {
                    ...
                    if (!empty($rs['service_ids'])) {
                        $svcNames = array_merge($svcNames, DB::table('services')->whereIn('id', $rs['service_ids'])->pluck('name')->toArray());
                    }
                    if (!empty($rs['subservice_ids'])) {
                        $svcNames = array_merge($svcNames, DB::table('subservices')->whereIn('id', $rs['subservice_ids'])->pluck('name')->toArray());
                    }
                    $step->recommended_services = implode(', ', $svcNames);
                }
            } else {
    ...
    public function updateWorkflow(Request $request, int $id): JsonResponse
    {
        ...
        DB::transaction(function () use ($oldWorkflow, $request, $steps) {
            ...
            // 3. Mark old workflow as no longer latest
            $oldWorkflow->update(['is_latest' => false]);
            ...
            DB::table('application_logs')->insert([
                ...
                'remarks'          => "Workflow '{$oldWorkflow->workflow_name}' updated from v{$oldWorkflow->version} to v{$newWorkflow->version}",
                'created_at'       => now(),
                'updated_at'       => now(),
            ]);
        });
    ...
    public function rollbackWorkflow(Request $request, int $id): JsonResponse
    {
        ...
        DB::transaction(function () use ($current, $previous) {
            ...
            DB::table('application_logs')->insert([
                ...
                'remarks'          => "Workflow '{$current->workflow_name}' rolled back from v{$current->version} to v{$previous->version}",
                'created_at'       => now(),
                'updated_at'       => now(),
            ]);
        });
